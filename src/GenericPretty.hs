...
instance PrettyShow Time.Day where
  prettyShow =
    LStr . Time.formatTime Time.defaultTimeLocale "%F"
...
instance
  (GPrettyShow f) =>
  GPrettyShowIgnoreConstr (S1 c f)
  where
...
instance
  (Selector s, GPrettyShow f) =>
  GPrettyShowAux (S1 s f)
  where
