...
import qualified Data.Aeson as Ae (Value, (.=), object)
...
handleHTTPError (InvalidUrlException _ _) = pure $ Left "invalid URL"
...
sendRequest h takesJSON r = do
    ...
    case parsedReq of
        Nothing ->
            pure . Left $ "Couldn't parse the HTTP request" ++ req
        Just parsedReq' ->
            (fmap (Right . getResponseBody) . httpLBS) parsedReq' `catches`
...
buildRequest takesJSON (Req method url params) =
  ...
  where
    reqWithoutJSON =
        show method ++ ' ' : T.unpack url ++ makeParamsString params
