...
module Telegram.Types.MediaGroup
...
instance ToJSON TlInputMediaPhoto where
    toJSON ph =
        let captionParam =
                maybe [] (\x -> ["caption" .= x]) $ impCaption ph
         in object $
            ["type" .= ("photo" :: T.Text), "media" .= impMedia ph] ++
            captionParam
...
instance ToJSON TlInputMediaVideo where
    toJSON ph =
        let captionParam =
                maybe [] (\x -> ["caption" .= x]) $ imvCaption ph
         in object $
            ["type" .= ("video" :: T.Text), "media" .= imvMedia ph] ++
            captionParam
...
instance ToJSON TlInputMediaDocument where
    toJSON ph =
        let captionParam =
                maybe [] (\x -> ["caption" .= x]) $ imdCaption ph
         in object $
            ["type" .= ("document" :: T.Text), "media" .= imdMedia ph] ++
            captionParam
...
instance ToJSON TlInputMediaAudio where
    toJSON ph =
        let captionParam =
                maybe [] (\x -> ["caption" .= x]) $ imaudioCaption ph
         in object $
            [ "type" .= ("audio" :: T.Text)
...
photoVideoCaption (TlpvPhoto (TlInputMediaPhoto {..})) = impCaption
photoVideoCaption (TlpvVideo (TlInputMediaVideo {..})) = imvCaption
...
maybeMediaGroupUnit m =
    let mCaption = messageCaption m
        mPhoto = messagePhoto m >>= S.safeHead :: Maybe TlPhotoSize
        mVideo = messageVideo m
        mDocument = messageDocument m
        mAudio = messageAudio m
        mInputMediaPhoto =
            fmap (TlInputMediaPhoto mCaption . photosizeFileID) mPhoto
        mInputMediaVideo =
            fmap (TlInputMediaVideo mCaption . videoFileID) mVideo
        mInputMediaDocument =
            fmap
                (TlInputMediaDocument mCaption . documentFileID)
                mDocument
        mInputMediaAudio =
            fmap (TlInputMediaAudio mCaption . audioFileID) mAudio
     in asum
