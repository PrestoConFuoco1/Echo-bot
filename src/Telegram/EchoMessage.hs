...
echoMessageTele m =
    maybe
        (Left $ "Failed to handle message." ++ GP.defaultPretty m)
        Right $
    asum $
    map ($ m)
        [ sendDiceTele
...
sendAudioTele =
    sendWithCaption messageAudio audioFileID "sendAudio" "audio"
...
sendVideoTele =
    sendWithCaption messageVideo videoFileID "sendVideo" "video"
...
sendVoiceTele =
    sendWithCaption messageVoice voiceFileID "sendVoice" "voice"
...
sendVenueTele m = do
    ...
    Just
        ( "sendVenue"
        , [ unit "chat_id" $ chatIDfromMsg m
          , unit "latitude" $ locationLatitude . venueLocation $ venue
          , unit "longitude" $
